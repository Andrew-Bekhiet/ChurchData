...
exports.exportToExcel = functions.runWith({memory:'512MB', timeoutSeconds:540}).https.onCall(async (data, context) => {
    ...
    const persons = (data?.onlyArea ?
        ...
        rslt['Study Year'] = studyYears[(p.data()['StudyYear'] as admin.firestore.DocumentReference)?.id];
        rslt['College'] = colleges[(p.data()['College'] as admin.firestore.DocumentReference)?.id];
        rslt['Job'] = jobs[(p.data()['Job'] as admin.firestore.DocumentReference)?.id];
        ...
        rslt['Serving Area'] = areas[(p.data()['ServingAreaId'] as admin.firestore.DocumentReference)?.id];
        rslt['Church'] = churches[(p.data()['Church'] as admin.firestore.DocumentReference)?.id];
        rslt['Meeting'] = p.data()['Meeting'];
        rslt['Confession Father'] = cfathers[(p.data()['CFather'] as admin.firestore.DocumentReference)?.id];
        rslt['State'] = states[(p.data()['State'] as admin.firestore.DocumentReference)?.id];
        rslt['Serving Type'] = servingTypes[(p.data()['ServingType'] as admin.firestore.DocumentReference)?.id];
